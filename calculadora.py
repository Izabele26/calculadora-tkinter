...
def inserir(valor):
    display.insert(END, valor)#end fica no primeiro argumento pois o valor inserido é apos o final do texto do display

def calcular():
    texto_display = display.get()
    resultado = eval(texto_display)#função para ve qual operação esta na string e calcula o resultado
    limpar()
...
display = Entry(window,font='Arial 20 bold', bg='#1c3ec7',
                fg='white', width=19)
display.pack()#adicionar o arquivo na janela

#todos os botoes ficam nesse panel
panel = Frame(window)
#criando botoes
btn_0 = Button(panel,bg='orange',bd=0,text='0',
               font='Arial 16 bold',width=5,height=3,command=lambda: inserir('0'))#lambda oculta o numero
...
window.mainloop()#sempre fica no final